...
from .types import infer_type_from_values, is_null_token, looks_like_padded_numeric
...
def _guess_format_hint(inferred_type: str, non_empty_values: list) -> str:
    ...
    of silently saying nothing. Every non-empty return value here is
    rendered as a "NEEDS REVIEW" flag, not routine information —
    see render_draft_yaml().
    """
    if inferred_type != "string" or not non_empty_values:
        return ""

    if looks_like_padded_numeric(non_empty_values):
        return (
            "looks numeric, but at least one value has a leading "
            "zero (e.g. a zip code or padded ID) — kept as string "
            "rather than integer/decimal to avoid corrupting it; "
            "change the type only if you're sure the padding doesn't matter"
        )
...
def flagged_fields(discovered: DiscoveredDataset) -> dict:
    """
    Fields `structifact discover` was NOT confident about, mapped to
    a one-line reason — every column with either no invertible type
    at all (`inferred_type == "unknown"`, meaning every sampled value
    was blank) or a format_hint (a "string" fallback discover can
    explain a specific reason for, e.g. inconsistent currency/date
    formatting or a leading-zero-padded identifier). This is the
    single source of truth for what render_draft_yaml() marks as
    "NEEDS REVIEW" and what the CLI prints as its own summary line —
    kept as one function so the two can never drift out of sync.

    Deliberately NOT included: a field simply being type "string"
    with no format_hint (the ordinary, unremarkable case — most
    string columns are just text), or a plain TODO description
    (every field gets one; it isn't a signal). A human reviewer can
    still disagree with an unflagged field's inferred type — this
    only surfaces the cases discover.py itself has a specific,
    nameable reason to be unsure about, not a claim that every
    unflagged field is definitely correct.
    """
    reasons = {}
    for f in discovered.fields:
        if f.inferred_type == "unknown":
            reasons[f.name] = (
                "every sampled value was blank — could not infer a "
                "type at all"
            )
        elif f.format_hint:
            reasons[f.name] = f.format_hint
    return reasons
...
def render_draft_yaml(discovered: DiscoveredDataset, ai_suggestions: dict = None) -> str:
    ...
    Every field the model was NOT confident about (see
    flagged_fields()) gets its own "NEEDS REVIEW" comment line,
    separate from the routine sample-count/blank-count/key hints —
    deliberately not folded into the same comment, so a flagged
    field is visually distinguishable from an ordinary one rather
    than requiring a human to parse a dense hint string to notice it.
    A top-of-file summary lists every flagged field by name, so the
    file's overall confidence is visible without reading every field.
    """
    ...
    if flagged:
        header += [
            "#",
            f"# ⚠ {len(flagged)} field(s) flagged for review: "
            f"{', '.join(flagged)}.",
            '# Search for "NEEDS REVIEW" below for why each one was flagged.',
        ]
    ...
    for f in discovered.fields:
        ...
        if f.name in flagged:
            lines.append(f"    # ⚠ NEEDS REVIEW: {flagged[f.name]}")
