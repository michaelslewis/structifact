...
a real, evidenced field-omission gap, distinct from the formatting-awareness item above — found retesting the third real-world requirements ticket (a SAP-shaped customer-credit source) after `AggregateRule` shipped: `discover --ai`'s draft silently omitted six real dimension fields (`struct_s066_ssour`/`vrsio`/`cmwae`, `struct_s067_ssour`/`vrsio`/`cmwae`) that are genuinely present, plainly, in the raw extracted requirements text — not excluded by any visible signal (no grey-fill, no separate out-of-scope table, unlike every other omission this project has previously found and explained). Not yet investigated: whether this is a prompt-construction issue, a genuine LLM extraction miss, or something about how these six fields' rows are laid out in the source spreadsheet (they sit in columns 3/4 of a 6-wide multi-table grid layout, alongside the fields that *were* correctly extracted from the same folders) that made them easy to miss. See DECISION_HISTORY.md's "Closing the Loop" note for the full account
