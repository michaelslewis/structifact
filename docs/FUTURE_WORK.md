...
a real, evidenced extraction-instability gap, distinct from the formatting-awareness item above — found retesting the third real-world requirements ticket (a SAP-shaped customer-credit source) after `AggregateRule` shipped. `discover --ai`'s first-pass draft omitted six real dimension fields (`struct_s066_ssour`/`vrsio`/`cmwae`, `struct_s067_ssour`/`vrsio`/`cmwae`) that are genuinely present, unambiguously, in the raw extracted requirements text — confirmed to be a clean extraction with no mechanical cause (checked the actual `.xlsx` cells directly with `openpyxl`: no merged-cell corruption, correct column/row positions) and no filtering anywhere in `discover.py`'s parsing/rendering code. Re-running the identical prompt against the identical document did NOT reproduce the same omission — it's genuinely non-deterministic, not a consistent bug — but the second run had its own, different failure mode: it correctly picked up the six previously-missing dimension fields, but lost the "this is an aggregate" semantic on six measures (`sum_oeikw` became `oeikw`, indistinguishable from a raw, non-aggregated column) and incorrectly added sixteen raw/join-only columns the document's own dimension/measure marker column explicitly leaves blank (meaning "not an output field"). Two independent runs, two different, non-overlapping ways of getting the same underlying document wrong — real evidence that this document's shape (a wide, sparse, six-way-repeated pipe-flattened grid, where a blank-vs-marked column is the only signal separating real output fields from join/filter/pre-aggregation-input columns) is genuinely difficult for the current prompt to extract reliably, not a single fixable one-off. Not yet designed against — a fix would need a second differently-shaped example showing the same instability, per this project's usual discipline, or a deliberate choice to accept single-document non-determinism as an inherent AI-extraction limitation rather than a bug to chase. See DECISION_HISTORY.md's "Closing the Loop" note for the full two-run comparison

a real, mechanical gap found scoping a fourth real example (`deliveries`, ~500 fields, a materially larger document than any of the first three), now fixed — `discover --ai`'s response was cut off entirely by `AnthropicLLMClient`'s `_APPROX_OUTPUT_TOKEN_CAP`, and raising the cap alone wasn't a complete fix either: a diagnostic attempt at a higher cap was rejected by the Anthropic SDK itself before any request was sent (requests estimated to run past 10 minutes require streaming, which `complete()` didn't implement). `complete()` now uses the streaming API (with visible progress feedback), and the cap is raised to 64,000 with real headroom over the ~27,000 tokens this document actually needs. Verified against the real document that originally failed: it now completes without truncation.

Fixing that surfaced two further, genuinely new findings from the same document, both still open:

* **`discover --ai` does not apply a later section's override of an earlier-declared field's attributes.** `deliveries`' requirements document defines fields once, then has a later "UPDATED FIELD NAMES, COMMENTS, AND FIELD DESCRIPTIONS" section giving a different `description` (and a `comment`) for specific already-named fields, while their role/type/length from the first section still apply. Once the response could actually complete (see above), the extracted `description` for an override-affected field (`struct_likp_stafo`) was confirmed to be the *original* main-section value, not the override section's replacement — the AI never applied the override at all. This is the actual capability the `deliveries` document was chosen to test, distinct from the mechanical token-cap/streaming gap that had to be fixed first just to observe it. Not yet designed against — needs its own real paper-contract pass (how should an extraction prompt represent "this later block modifies an already-named field," given the cross-referencing itself is non-trivial — the override section's own field names use a different prefix, `sap_*` instead of `struct_*`, matched only by a shared suffix).
* **`build_requirements_prompt()`'s extraction schema doesn't ask for `comment` at all.** Found while investigating the override gap above: even if the merge worked, `discover --ai` couldn't surface a `comment` value today regardless, since the prompt's own field schema (`name`/`description`/`role`/`type`/`computed`/`expression`/`note`) was never updated after `FieldSpec.comment` was added to the IR. A smaller, more mechanical fix than the override-merge question, but deliberately grouped with it rather than patched in isolation, since a real test of "does the override section's comment surface correctly" needs both fixed together to mean anything.
