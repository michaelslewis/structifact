...
a real, mechanical (not fuzzy) gap found scoping a fourth real example (`deliveries`, ~500 fields, a materially larger document than any of the first three) — `discover --ai`'s response was cut off entirely by `AnthropicLLMClient`'s `_APPROX_OUTPUT_TOKEN_CAP`, which had already been raised twice before (500 → 4000 → 8000) in response to real truncations, but on documents an order of magnitude smaller. A rough estimate from this document's prompt size put the output actually needed at ~27,000 tokens — well past the current cap. Raising the cap alone isn't a complete fix, though: a diagnostic attempt at `max_tokens=32000` was rejected by the Anthropic SDK itself before any request was sent (`ValueError: Streaming is required for operations that may take longer than 10 minutes`) — `complete()` only ever makes a plain non-streaming call. So a document this size needs real streaming support in `AnthropicLLMClient`, not just a bigger constant. Deliberately not implemented yet — logged as a real, evidenced need, its own future paper-contract candidate (client-side streaming support, likely also progress feedback during a long-running extraction) rather than a same-session patch. The temporary diagnostic cap change was reverted, not committed.
