...
**Previously-discussed second complex XLSX/YAML example — resolved without building it.** The underlying messy-XLSX discovery capability is already evidenced, most concretely by `examples/coverage_round1/requirements_docs/medium_subscription_billing.xlsx`. See `DECISION_HISTORY.md` for the evaluation and decision. The separate question of whether to present that existing evidence on structifact.com remains open.
...
* **`discover --requirements --ai`'s inference of a shared physical `source_column` across multiple role-joins of the same table is not consistent across documents — logged, not scoped.** `examples/workorder_demo`'s `PARTNER_ROLE` extraction (a table joined three times under different roles) correctly inferred ONE shared physical column (`source_column: "contact_name"`) reused identically across all three role sources — the realistic shape for "one physical table, several logical parties," which neither document states explicitly; the model has to infer it. A second, structurally similar real coverage document (`examples/coverage_round1/requirements_docs/hard_insurance_claims.md`, `PARTY_ROLE` joined three times under `CLAIMANT`/`ADJUSTER`/`BENEFICIARY`) instead assumed a separate physical column per role (`claimant_name`, `adjuster_name`, `adjuster_email`, `beneficiary_name`) — a wide-table-with-one-column-per-role shape, rather than one shared column varying by row. Both are structurally valid — `structifact validate` accepts either shape, and both execute correctly against synthetic data built to match each assumption — but they represent genuinely different assumptions about the underlying physical schema, produced from the same class of ambiguous input across two different real documents. Not yet understood as a prompt-wording issue versus a genuine model-capability inconsistency — matching how the blank-role-marker and override-merge findings above were characterized before being understood further. See `examples/coverage_round1/REPORT.md` for the full account.
...
inline validation — **partially real, no custom extension code involved, and confirmed working by direct human observation in the editor (not just automated testing).** `schemas/structifact-dataset.schema.json` (see DECISION_HISTORY.md) is a JSON Schema for the hand-authored dataset YAML, wired into `.vscode/settings.json` for the Red Hat YAML extension (`redhat.vscode-yaml`) — live squiggles/autocomplete for the schema-expressible subset of `structifact/validation.py`'s rules (required fields, enums, non-empty arrays, mutually-exclusive pairs, same-object conditionals like `computed`/`expression`) while editing, no CLI run needed. The wiring itself, plus three specific rule categories — a bare required-property violation, an enum violation (with the correct allowed-values list shown), and a same-object conditional shape rule (`foreign_key` requiring `target_table`/`target_column`) — were verified by hand in a real VS Code window against an untracked test copy, not just by the automated `jsonschema`-library pass described in the previous DECISION_HISTORY.md entry; see the later entry for exactly what was and wasn't covered by that manual pass (most of the schema's other rules — `dedup`/`aggregate` exclusivity, the rest of the `computed`/`expression`/`depends_on` conditionals, `uniqueItems`, etc. — remain verified only by the automated pass, not by hand). Deliberately does NOT cover this rule set's cross-reference/cross-item checks (a join's `source` naming a real declared source, a foreign key's target existing, duplicate `depends_on` across a collection, more-than-one `primary_key`) — those still require actually running `structifact validate`, which is the real motivation for the subprocess/CLI-invocation idea below remaining on this list, scoped now specifically to that smaller remaining rule set rather than validation in general.
...
**Open Thread 1, revisited: this entry originally conflated two distinct gaps found in the same investigation. Split below now that one is resolved.**

**Sub-problem A — cross-join ranking: CLOSED, superseded by `JoinSpec.pick_one_order_by`.** `DedupRule`'s `ROW_NUMBER()` CTE (`model.py`'s `_source_cte`) is built only from the joined-in source's own columns, before any join happens, so it could not express "the row applicable as of a date/value on the *primary* table." First identified in `examples/value_experiment/experiment_b_notes.md` (forced a three-file pipeline workaround); independently reproduced a second time in `examples/coverage_round1/`'s `hard_insurance_claims` as silent row duplication, a materially worse failure mode with no workaround applied. Both reproductions are now resolved: `JoinSpec.pick_one_order_by` (commit `569c97b`, `docs/PICK_ONE_ORDER_BY_CONTRACT.md`) ranks and selects exactly one row inside the join's own scope, where the primary source is already visible — see the contract's §9 for the one-line fix applied to each real reproduction, and `examples/value_experiment/order_status_and_revenue_pick_one_order_by.yml` for the collapsed single-dataset version of the original three-file pipeline. No further design work needed here.

**Sub-problem B — primary-source reduction/collapse has no explicit IR representation: still OPEN, single example, deferred.** `dedup`/`aggregate` exist only as fields on `SourceRef` (`ir.py`) — confirmed directly, not just by example usage: `DatasetSpec` has no such field, `SourceRef`'s own docstring describes it as representing a joined-in instance of a table, and `_source_cte()` (the only code path that applies a `DedupRule`/`AggregateRule`) is called exclusively over `dataset.sources`, never over the primary source. There is no first-class way to collapse the *primary* source to one row per key with no join involved. The one known workaround (`examples/value_experiment/order_status_resolved.yml`, `order_status_revenue_summary.yml`): set `source_table` equal to a `SourceRef`'s own `name`, with an empty `joins:` list, exploiting `ModelGenerator`'s name-based CTE resolution rather than any declared IR relationship. Checked directly: every real occurrence of this workaround in the repo belongs to the same `value_experiment` pipeline — one motivating case, not two independent reproductions, so this does not yet meet the bar (`DESIGN_PRINCIPLES.md` #10, #18; the same bar `AggregateRule`/`source_filter`/`pick_one_order_by` all had to clear) that would justify scoping a fix. Deliberately not designed further, and deliberately not pre-assumed to need a `DedupRule`-shaped solution — the right abstraction, if a second independent case ever justifies building one, might be a generalized source-reduction operation, an extension of existing `dedup`/`aggregate` semantics to the primary source, or something else; that question should be answered by the second case, not guessed at now from one.

**Resolved — see `JoinSpec.pick_one_order_by` (commit `569c97b`,
`docs/PICK_ONE_ORDER_BY_CONTRACT.md`).** Rather than modify `DedupRule`
itself, a new `JoinSpec.pick_one_order_by` field ranks and selects
exactly one qualifying row inside the *join's* own scope (via a
correlated `LEFT JOIN LATERAL`), where the primary source and any
earlier-declared join are already visible — the same scope `on`
already had. Both real reproductions above are directly addressed:
see the contract document's §9 for the one-line fix applied to each.
`DedupRule`/`SourceRef` themselves are unchanged; this does not
reopen `DedupRule` as an unresolved architectural gap.

**A computed field referencing another computed field's own `SELECT`-list alias works on DuckDB but is confirmed to fail on PostgreSQL once the referenced alias's own expression is a subquery — a real, verified portability gap, not previously documented. Logged, not scoped.** `ModelGenerator`'s `_select_line` inlines a computed field's `expression` as-is, with no qualification and no check that whatever it references actually resolves at that `SELECT` level. Found investigating `examples/coverage_round1/requirements_docs/medium_subscription_billing.generated_model.sql`: after hand-patching one computed field's prose expression to real SQL, a second computed field (`monthly_price_usd`) whose own expression references the first field's alias (`resolved_fx_rate`) directly executed successfully on DuckDB and produced correct values — but only while `resolved_fx_rate`'s own expression stayed a scalar `CASE`/`COALESCE`. Once `resolved_fx_rate`'s expression became a scalar subquery instead (needed to test the aggregation case in the `DedupRule` finding above), the identical sibling-alias reference failed outright, even on DuckDB, with its own `BinderException`: `"Alias referenced in a SELECT clause - but the expression has a subquery. This is not yet supported."` Separately, the plain (non-subquery) sibling-alias case was reproduced directly against a real local PostgreSQL server: PostgreSQL rejects it unconditionally with `column "resolved_fx_rate" does not exist` — referencing another column's own `SELECT`-list alias within the same `SELECT` is not standard SQL, and works on DuckDB only as a non-standard convenience. Nothing in `ModelGenerator` prevents a computed expression from relying on this, since expressions are trusted, unparsed raw text — so any generated SQL that happens to work this way is silently non-portable across the two engines Structifact already supports.
