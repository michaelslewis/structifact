...
**`DedupRule` cannot rank rows using a column from the primary table — first found as friction, now independently reproduced as silent data corruption. Logged, not scoped.** `DedupRule`'s `ROW_NUMBER()` CTE (`model.py`'s `_source_cte`) is built only from the joined-in source's own columns, before any join happens, so it cannot express "the row applicable as of a date/value on the *primary* table" — only "the single best row for this key, full stop." First identified in `examples/value_experiment/experiment_b_notes.md`: resolving a customer's status as of an order's own date required abandoning `DedupRule` for the join in question and manually splitting the dataset into a multi-stage pipeline (a fan-out dataset, then a second dataset applying `DedupRule` to the already-materialized result) — real friction, but no wrong output, since the workaround was applied deliberately by a human.

A second, independent real reproduction (`examples/coverage_round1/`, `hard_insurance_claims.md`) found a materially worse manifestation of the same root cause, produced automatically rather than worked around: `discover --requirements --ai` itself attempted a shortcut for the identical as-of-date pattern, folding the date condition directly into a join's `"on"` clause as an inequality (`"CLAIM_HDR.policy_id = policy_status.policy_id and policy_status.effective_date <= CLAIM_HDR.claim_date"`) instead of using `DedupRule`, with no corresponding row-reduction. Nothing rejects this — `structifact validate` passes, and `validation.py` has no rule checking whether a join with no `dedup` could plausibly match more than one row per primary-table key. Confirmed against real synthetic data (`examples/coverage_round1/requirements_docs/hard_insurance_claims.generated_model.sql`, executed in DuckDB): a claim whose policy has two `POLICY_STATUS_HISTORY` rows both satisfying the inequality came back as **two output rows** — one correct, one a stale duplicate — with `total_paid_amount`/`net_exposure` duplicated onto the phantom row too. No error, no warning; any aggregate built on top would silently double-count. Two independent real reproductions of the same underlying gap now exist, with two different severities (workflow friction vs. silent row duplication).
