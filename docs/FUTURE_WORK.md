...
**Aggregated joined sources — a second, distinct gap in the sources/joins model, found via a third real-world requirements ticket (a SAP-shaped customer-credit source; see DECISION_HISTORY.md's "Real-World Validation" entries).** Today, `SourceRef`/`JoinSpec`/`DedupRule` assume every joined-in source is already at 1:1 cardinality with the primary source — either naturally, or reduced to one row per key via `DedupRule`'s `ROW_NUMBER()`-based priority selection. The real reference SQL needed two materially different aggregation shapes neither mechanism covers: (1) a joined source pre-aggregated (`SUM(...) GROUP BY <keys>`) into its own CTE *before* being joined in, so it arrives at the join already collapsed to one row per key; and (2) a joined source left un-aggregated at join time, with a measure computed via a conditional sign-flip (`CASE WHEN flag = 'S' THEN amount WHEN flag = 'H' THEN -amount ELSE 0 END`) and summed via a `GROUP BY` spanning every other selected column at the *final* select level — collapsing a one-to-many join at the very end rather than before it. `ModelGenerator`'s existing `computed`/`expression` escape hatch cannot absorb either shape: today's generated model is always a flat `SELECT` with `LEFT JOIN`s and no `GROUP BY` anywhere, so a bare `SUM(...)` inside a computed field's raw expression would produce invalid SQL once mixed with non-aggregated columns in the same ungrouped select. Deliberately not scoped or designed here — this is a real, evidenced gap (confirmed directly against real reference SQL, not a hypothetical), but it's materially bigger than any single-field IR addition this project has made so far (it touches core join/select-shape generation, not just metadata), and deserves its own real paper-contract pass, the same discipline `source_filter` and reconciliation each got, rather than being patched in as an addendum to the round that found it.
