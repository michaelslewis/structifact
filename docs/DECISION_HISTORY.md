...
After the 1.0 readiness audit closed with an empty backlog, the agreed next step was to stop building from the roadmap and wait for real use to surface a real need. That happened almost immediately: a real-world dataset (a SAP-shaped segment-master data source — one requirements xlsx, plus SQL/dbt-YAML/catalog already hand-built as ground truth, kept entirely outside the git repo in a scratchpad directory since it's real-world data) was run through Structifact's actual pipeline as a genuinely adversarial test, not a demo built to look good.
...
`source_field` was corrected to the simpler, now-doubly-confirmed rule. `ModelGenerator` gained a fourth condition (`has_renaming`, alongside computed/sources/filter) to its "generate or return None" check. Both were verified against real DuckDB (and, for the model-execution case, PostgreSQL) data, and the full `job_item_master` fixture was hand-modeled and run through `validate`/`generate` end to end — its model SQL, dbt YAML, dbt_extended YAML, and catalog CSV all now match the real hand-built reference files closely, the same standard applied to the first example.

One more real-world wrinkle, unrelated to Structifact: the reference SQL, the reference YAML, and the catalog CSV for this dataset did not all agree on the same field list. Reference material drifting out of sync with itself is ordinary and expected — it is, in fact, much of why a single-source-of-truth tool like this one is worth building at all — so the acceptance fixture was scoped to the fields all three agreed on. Recorded for the methodological point only: not every discrepancy an adversarial real-world test turns up is a Structifact question, and the right response is to scope around it rather than assume the tool is at fault.
...
**Follow-up:** fixed shortly after, once explicitly requested — `_sql_type()` gained a `string`-with-`length` branch emitting `VARCHAR(length)`, mirroring the existing `decimal`-with-`precision`/`scale` branch exactly (same gating: only when the value is actually set, otherwise the existing bare-type fallback is unchanged). Verified against the real Account Summary fixture's DDL, not just the synthetic test suite. See `ROADMAP.md`'s "Recently Completed" list.
...
The third real-world-validation round's headline finding (see the entry above) was that `SourceRef`/`JoinSpec`/`DedupRule` had no way to express a joined source needing `SUM(...) GROUP BY` — deliberately left unscoped and unimplemented in that same round. Once explicitly asked to scope and build it, the real reference SQL's two aggregation-dependent joined sources were re-examined side by side: `aggopen`/`aggdue` were already written as pre-aggregated CTEs (`SUM(...) GROUP BY <keys>`, joined normally afterward), while `openitem` was joined raw and aggregated via a `GROUP BY` spanning the *entire final select* — a materially different-looking shape, collapsing a one-to-many join at the end rather than before it.

Working through the semantics on paper rather than assuming two mechanisms were needed: `openitem`'s final-level `GROUP BY` only "works" because `credctrl` (the primary source) is already unique per `(custid, ctrlarea)` — the same grain `openitem` would be aggregated to if it were pre-aggregated the way `aggopen`/`aggdue` already are. Once that's true, joining a pre-aggregated `openitem` (grouped by `custid, ctrlarea`, with the same conditional sign-flip `SUM` as the aggregate expression) to `credctrl` produces an identical result set to the reference SQL's join-then-final-`GROUP BY` approach — the two shapes are mathematically equivalent for this data, not just superficially similar. That meant the two "distinct" real requirements the previous entry described were actually one requirement observed twice, in two different but equivalent hand-written forms.
...
With `AggregateRule` built, the full `account_summary` dataset — all 50 real reference fields, not just the 35 that didn't need aggregation — was hand-modeled and re-run through the same comparison process. Every field's role/description/`source_field`, and every dataset-level `dbt_extended` field, matched the real reference exactly (verified programmatically, the same script used for the first pass). One methodological point along the way: where two structurally similar aggregated fields carried different dimension/measure markings in the reference material, the hand-model was corrected to reproduce the reference *exactly* rather than normalizing it to what seemed more internally consistent. That is the correct behavior for an acceptance fixture — its job is to encode what the reference actually says, not an improved version of it. Silently "fixing" reference material while using it as ground truth would make any subsequent match meaningless.

**Correction (found while scoping a fourth real example):** the "matched exactly" claim above was true only for the attributes actually checked — `description`, `role`, and `source_field`. It did not check `comment`, a fourth `meta` attribute the real reference file uses on 42 of its 50 fields (e.g. `comment: Customer Group` alongside `description: Customer Group (Extended)`). `FieldSpec` has no `comment` attribute at all, and neither `DBTYAMLGenerator` nor `DBTExtendedYAMLGenerator` emits one — this was never a close call that got fixed, it's an attribute Structifact has no representation for whatsoever. It went unnoticed through both this round and the regression check against `segment_master`/`job_item_master` because neither of those two reference files uses `comment` even once — the gap genuinely couldn't have been caught by either of the first two real examples, only by a diff broad enough to check every attribute the reference actually carries, not just the ones already known to matter.

Beyond the field-metadata match, the generated model was executed end-to-end against synthetic data specifically designed to prove the computed fields derived from multiple pre-aggregated sources come out numerically correct, not just that the SQL runs: a customer with a debit and a credit `openitem` row (sign-flip aggregation), one `aggopen` row, and one `aggdue` row produced `struct_credit_exposure` and `struct_credit_limit_used_percent` values matching hand-calculated expectations exactly.

One new, real, but separately-scoped finding surfaced during the retest, unrelated to aggregation: comparing the full 50-field reference against the earlier `discover --ai` draft (44 fields) showed the AI extraction had silently omitted six real dimension columns (`struct_aggopen_srckey`/`versionkey`/`currcode`, `struct_aggdue_srckey`/`versionkey`/`currcode`) that are genuinely present, unambiguously, in the raw requirements xlsx text — not excluded by any visible logic the other omissions in this document's earlier entries had (grey-fill formatting, out-of-scope tables). This wasn't caught during the original round because that round's comparison focused on the 44-field draft's own internal consistency, not a field-count audit against the full reference. Logged as a real, evidenced `discover --ai` gap, not yet investigated or fixed — see `FUTURE_WORK.md`.
...
Scoping a fourth real example (`shipment_header`, a materially larger and structurally different ticket — a delivery-header/item dataset assembled from four joined SAP tables, its requirements document containing a later-added section that overrides specific fields' descriptions and adds comments while leaving their earlier-declared role/type/length untouched) surfaced that its reference catalog CSV uses a non-blank `comments` column throughout. That prompted checking `account_summary`'s reference file too, not just assuming it was `shipment_header`-specific: 42 of its 50 fields also carry a `comment` value in `meta`, distinct from `description` — a real attribute this project had already generated output for (twice) without ever comparing against.

This had gone completely undetected through the third real example's own "full match" claim (see the correction appended to that entry above) for a concrete, explainable reason: `account_summary`'s 42 comment-bearing fields are *all* in the "General"/customer-master section that example deliberately excluded from its hand-model (out of scope, belonging to a separate `party_master_general` model never provided). The gap wasn't hiding — the fields that would have exposed it were never modeled in the first place.
...
Verified against a real reference field (`struct_custmstr_attr1` from the `account_summary` dataset, description "Customer Group (Extended)", comment "Customer Group") before being considered done, not just the synthetic test suite — both generators reproduced the exact real values.
...
**Follow-up — picked up in a later, explicitly requested round, not mid-round momentum.** Asked directly to fix this and the related run-to-run-instability finding, streaming support was added to `AnthropicLLMClient.complete()`, and `_APPROX_OUTPUT_TOKEN_CAP` raised to 64,000 — this time confirming the API actually accepted the value (a trivial, near-zero-cost prompt at `max_tokens=64000`) before hardcoding it, rather than assuming. Verified against the real `shipment_header` document that originally failed: the response completed without truncation.

That success surfaced a second real, previously-invisible bug in the same pass: the completed response failed `yaml.safe_load()` with "mapping values are not allowed here," traced to an unquoted colon inside a real field description (`"Account Status: Risk Tier"` — YAML interprets a bare colon as starting a new mapping key). The same class of problem this project already solved once for markdown code fences (`_strip_code_fence`) — the prompt asked for valid YAML, which is an instruction, not a guarantee. Fixed by having `build_requirements_prompt()` explicitly require every string value to be double-quoted. Re-verified against the same real document a third time: the response now parses successfully end to end, confirming both fixes together, not just each in isolation.

With the mechanical path finally clear, the actual question `shipment_header` was chosen to answer got answered for real: `discover --ai` does not apply the later section's override to an already-named field — the extracted description for `struct_delivhdr_statgroup` was confirmed to be the original main-section value, not the override section's replacement, checked directly against the real completed response rather than assumed from the earlier truncated ones. A related, smaller gap surfaced investigating this: `build_requirements_prompt()`'s own extraction schema never asks for `comment`, so a working override merge couldn't have surfaced one via this path regardless. Both logged in `FUTURE_WORK.md`, deliberately not designed against here — this entry's original point about not fixing something under mid-round momentum still held for *these* two findings, even though the token-cap/streaming fix itself was picked up in the same conversation once directly requested.

**Second follow-up — explicit instructions for both the comment field and the override-merge pattern were added, then verified against the real document again, and neither worked.** Sampled two override-affected fields (`struct_delivhdr_statgroup`, `struct_delivitem_updflow`); both still showed the original main-section value, not the override section's replacement, and zero `comment` values were extracted anywhere in the 366-field response — expected, since in this document `comment` values only ever come from the override section, which was never actually applied. Worse than a silent miss: the model's own `unresolved_notes` explicitly claimed "all revisions have been applied to primary field entries" — a confident, false statement, checked and disproven directly rather than trusted. A reviewer relying on that note, rather than checking the actual field values, would have no reason to suspect anything was wrong.
...
**Third follow-up — a trimmed real excerpt isolated the variable, and answered the model-capability question, at real and poorly-managed cost.** Rather than re-running the full ~500-field document again, a much smaller excerpt was built from the *same real document*: the complete `DELIVHDR` field block plus only its own override entries (skipping `DELIVITEM`/`HDRSTATUS`/`ITEMSTATUS` and their overrides entirely) — real text, same prefix-matching challenge, roughly a third the size. Run against `claude-haiku-4-5`, it failed in a third, still-different way: rather than silently keeping the original value (the full-document result) or falsely claiming success, it created a *new, separate* field under the override section's own name (`sap_delivhdr_statgroup`) instead of merging into the existing `struct_delivhdr_statgroup`, kept the original main-section description, and misfiled the override's replacement description into `comment` instead. `struct_delivhdr_statgroup` itself didn't appear anywhere in the response — three real runs, three genuinely different failure shapes, all wrong. Run against `claude-sonnet-5` on the identical excerpt, `struct_delivhdr_statgroup` came back exactly right — correct name, the override section's description, the override section's other value correctly placed in `comment`, original role/type preserved. A real, positive answer to the actual diagnostic question: this is at least partly a model-capability threshold, not purely a prompt-wording problem.
...
**Arm A** (`claude-haiku-4-5`, shipped prompt, 40-field/4-override document, 5 runs): **5/5 `OK`.** **Arm C** (same document, same model, the shipped override-merge instruction removed locally and uncommitted, 5 runs): **2/5 `OK`, 3/5 `F-DUP`** — and the failure was not spread across the four overrides, it was the *same field every time*, `struct_delivitem_segcode`, the one override whose revision crosses from the `DELIVHDR` table into `DELIVITEM`. The other three same-table overrides succeeded in all ten runs regardless of the instruction's presence. So the shipped instruction (permanently added to `build_requirements_prompt()`, `discover.py` ~L393–408, during the earlier "Second follow-up" round) is not inert — it earns its place — but its measurable effect at this scale is narrower than "fixes overrides": it specifically rescues cross-table name correlation.

**Arm D** (shipped prompt, 500-field/12-override document — the same scale as the real `shipment_header` document, 3 runs): **3/3 `OK`**, clean across all twelve overrides. Read against `DECISION_HISTORY.md`'s own scale hypothesis, this looked like a real answer: haiku handles 500 fields as cleanly as 40.
...
**Arm E** (shipped prompt, 40-field hard document, 5 runs): **5/5 `OK`**, including `struct_delivitem_segcode` — the cross-table field that failed 3/5 times in Arm C — clean in every run. Removing the document's self-explaining hint made no difference at small scale, as long as the shipped prompt's own instruction was present.
...
# Decision: A Four-Fixture, Two-Model Reproducibility Pass on `discover --ai` — Technically Demonstrated, Not Commercially Validated (2026-09-07)

**Technically demonstrated ≠ commercially validated.** Everything below shows the underlying requirements-to-schema transformation is technically feasible and plausibly labor-saving for the one real manual data point collected against it. It shows nothing about whether this problem is common, painful, or valuable enough elsewhere to justify building a product around it — that question was not investigated in this round and is recorded as open, not answered, at the end of this entry.

## What Happened

Four synthetic, fully fictional requirements documents (no real employer data or systems) were built and run through the unmodified real `discover --ai` pipeline — `build_requirements_prompt()` → a real Anthropic API call → `parse_requirements_draft()` → `render_requirements_draft_yaml()` → `structifact validate` — with a cost estimate stated and explicit go-ahead obtained before every real API call, no exceptions. Five real API calls were made in total (one document, `order_extract.md`, was run twice — once per model). Where `claude-sonnet-5` was used instead of the shipped `claude-haiku-4-5` default, the model was overridden on the client *instance* only (`client.MODEL = "claude-sonnet-5"` in a throwaway script); `structifact/llm.py:104`'s hardcoded default was never edited.

**`order_extract.md`** (order header + one explicit CRM join, three fields with blank role markers, a lookup field whose join key is never named, a revisions section overriding two fields) — run twice, once per model, prompt and document held identical:

- **`claude-haiku-4-5`** (the shipped default): correctly extracted the explicit CRM join and merged the revisions section without duplication, but violated the prompt's own explicit blank-role-marker instruction — silently assigned `role: dimension` to three fields (`order_date`, `status`, `notes`) that had no role marker in the source — and fabricated a join condition, `ORDERHDR.shipping_zip = zip_region_map.zip`, where `shipping_zip` is a column name that appears nowhere in the document; the fabrication was placed directly in the structured `joins` block, with a separate `unresolved_notes` entry acknowledging the column name was inferred. Validated cleanly (9 fields).
- **`claude-sonnet-5`**: same document, same prompt, both of the above failures absent — the three blank-role fields were correctly excluded into `unresolved_notes`, and the zip lookup was left out of `sources`/`joins` entirely rather than invented. Also used the `comment` field for the revision-section rename (`cust_id` → "Customer Account Number") rather than a straight description overwrite, keeping the original description intact — a plausible, arguably better reading of the prompt's own `comment` definition than Haiku's overwrite. Validated cleanly (7 fields).

**Doc A, "Customer Account Snapshot"** (`claude-sonnet-5` only) — designed to test: multiple blank/contradictory role markers, a term ("account type") that means two different things in two different sections of the same document, a revisions section, and a relationship described only in prose with no join key given. Result: the three blank-role fields (`open_date`, `last_activity_date`, `flags`) were correctly excluded; the document's two "account type" senses (CHECKING/SAVINGS/MONEY_MARKET vs. PREPAID/POSTPAID) were kept separate rather than conflated; a genuine document-stated contradiction (`risk_score` marked `dim` in the table but described in prose as "actually a calculated measure... a copy-paste error") was correctly resolved to `role: measure`, since the document itself resolved it rather than leaving it ambiguous; the billing-profile relationship, described only in prose with no join key, was correctly left out of `sources`/`joins` and parked in `unresolved_notes`; the revisions section (`status_code`, `credit_limit`) merged cleanly with no duplication. Validated cleanly (7 fields).

**Doc B, "Shipment Summary"** (`claude-sonnet-5` only) — designed to test: multiple source tables, one explicit join plus one prose-only implicit join, a computed field, a renamed field, mixed typed/untyped fields, and three deliberate "plausible-but-unsupported inference" traps (a business rule about defaulting to a regional manager that isn't tied to any listed field; two CARRIER_MASTER attributes explicitly stated as "not part of this extract"; a background sentence — "all shipments in this extract originate from the WEST region" — that could tempt a model into fabricating a `filter` on a source). Result: all three traps were avoided — no invented `manager_id` field, no invented surcharge/rating fields, no invented `filter` clause on any source; the explicit carrier join was extracted correctly with the right `source_column`; the warehouse lookup, described only as "handled ad hoc... nobody has written down which columns it actually uses," was correctly left out of the structured join block; the pure rename (`carrier_name` → `comment: "Carrier"`) and the previously-undocumented `freight_cost` description from the revisions section were both handled without duplication or invention. Validated cleanly (10 fields).

**`doc_c_sap_shaped.md`, "Billing Extract"** (`claude-sonnet-5` only) — an SAP-shaped fixture: `sap_<table>_<field>` naming, tab-separated and pipe-separated field tables in inconsistent, ragged shapes (one row short a column, one row missing type/marker entirely), three source tables, a mix of stated and unstated types (including a type written as the source document's own `dec(13,3)` abbreviation rather than `decimal`), a computed field, an ambiguous business term (Sold-to vs. Ship-to party), two internal/technical fields explicitly marked as not reportable (`MANDT`, `LOEKZ`), a governance-flavored note that two customer fields are "classified as Restricted" with "no formal tagging mechanism... in the extract itself today," and a revisions section renaming one field's business label while also warning against confusing it with an unrelated, already-removed field (`GROSS_VALUE`). Result, in full:

- **Did not validate.** `structifact validate` failed: `Unsupported type 'dec(13,3)' for field 'sap_billitem_fkimg'`. The model transcribed the document's own stated type literally, per the prompt's instruction to use the document's own wording — the break is downstream, in `structifact/types.py`'s type-alias table, which maps `DECIMAL`/`NUMBER`/`NUMERIC` to `decimal` but has no entry for `DEC`, a common SAP/mainframe decimal abbreviation. This is a real, currently unfixed gap in `structifact/types.py`, not a prompt or model defect, and is **not fixed by this entry** — recorded as a `FUTURE_WORK.md` candidate only.
- Governance-flavored note handled correctly: no attempt to invent a nonexistent `pii` (or similarly-shaped) field on the IR — the two customer-contact fields' Restricted classification was correctly routed to `unresolved_notes` instead, respecting that the prompt's schema has no slot for it.
- Two internal/technical fields (`MANDT`, `LOEKZ`) and two ambiguous/incomplete rows (`sap_billhdr_ernam`, `sap_billitem_vrkme`) were correctly excluded from `fields` and explained in `unresolved_notes`.
- The revision section (`sap_billhdr_netwr` → "Extended Price") was placed in `comment`, description intact, plus the `GROSS_VALUE` non-confusion note preserved in `unresolved_notes`.
- **The one finding that matters most from this fixture:** the model fabricated two concrete join conditions directly into the structured, load-bearing `joins` block. The `ZBILLHDR`↔`ZBILLITEM` join (`VBELN`, `type: "inner"`) was never stated in the document at all — the key and the join type were both supplied from the model's own SAP domain knowledge, disclosed after the fact in `unresolved_notes` as "inferred from the standard SAP billing header/item relationship." Separately, and more consequentially, the sales-org lookup join (`ZBILLHDR.VKORG = sales_org_text.VKORG`) was emitted as a specific, confident `on:` condition even though the source document explicitly states the opposite of confidence — *"the exact join column names were never nailed down... whoever wrote the ETL just eyeballed it against sample data."* This is recorded as a named, reproduced failure mode distinct from the earlier `order_extract.md`/Haiku fabrication (an invented column name anyone would catch on inspection): here the fabricated join is textbook-plausible SAP structure, which makes it *harder* to catch on review, not easier, and it was emitted into the authoritative structured output at the same time the prose `unresolved_notes` correctly disclosed the inference — the two are in direct tension, and a reviewer who trusts the YAML's structural shape over reading every note would ship an unverified join as if it were sourced from the document.

## Model and Cost Data

`claude-haiku-4-5` is `AnthropicLLMClient`'s hardcoded, shipped default (`llm.py:104`); it is not configurable via any CLI flag. `claude-sonnet-5` was used in four of the five real API calls in this round, in every case via an instance-only attribute override in a throwaway script — `llm.py` was never edited, not even temporarily.

`AnthropicLLMClient.estimate_cost()` (`llm.py:148-166`) always prices its estimate against `_APPROX_OUTPUT_TOKEN_CAP` (64,000 output tokens) — a deliberate worst-case ceiling sized for a real ~500-field document, not a forecast of what any given call will actually produce. Every cost estimate shown before a real call in this round therefore displayed a number in the $0.32–$0.65 range; **that number was never the real charge for any of the five calls made.** Actual spend, approximated from each response's character length using the same ~4-characters-per-token heuristic `estimate_cost()` itself uses (real billed token counts via the API's own `usage` field were not separately captured in this round — this is stated as an approximation, not a verified billed figure): every completed call came in under $0.02, most in the $0.008–$0.017 range. Total real spend across all five calls in this round was on the order of a few cents, well under a dollar.

## The Manual Baseline

A real, honest manual attempt was made at translating `doc_c_sap_shaped.md` by hand, before the generated draft was revealed. It ran approximately 25 minutes and was explicitly incomplete when stopped: one join condition was left as an unfilled placeholder, a second join was not attempted at all, and no catalog/YAML output had been started. The honest estimate at the point of stopping was roughly one more hour of work to reach a state comparable to what the AI pipeline produced (draft YAML, joins attempted, catalog-shape reachable) — including, notably, the same unresolved-provenance question the AI pipeline also flagged for the VKORG lookup, since a careful human translator hits the same genuine unknown the source document itself admits to. **This is not a completed baseline and must not be read as one** — it is a single, partial, single-person data point on how long the equivalent manual work takes to *start*, not a measured completion time to compare against.

## What Remains Unknown

This entry records a technical finding only. It does not investigate, and takes no position on, whether:

- this problem (extracting a draft schema from a messy, human-authored requirements document) is common enough across other organizations, teams, or industries to matter beyond this one person's own past work experience;
- the manual-effort savings suggested by the one partial, 25-minute manual data point above would generalize to other documents, other authors, or other domains;
- anyone other than this project's author would find this capability valuable enough to pay for, adopt, or change a workflow around;
