...
Deliberately, explicitly NOT built yet — see `FUTURE_WORK.md`'s "Before a 1.0 Release" section: a real Snowflake (or other) engine implementation, real transaction/connection-pooling/retry handling (Postgres's `autocommit=True` is a stopgap, not that), materializing `ModelGenerator`'s output into a real target table, any CLI exposure for model execution, and proving the sources/joins/dedup CTE shape executes correctly.
