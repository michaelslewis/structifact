...
`structifact/generators/` — `SQLGenerator` (type-aware DDL, now including `FOREIGN KEY`/`CHECK` constraint emission), `DBTYAMLGenerator`, two catalog generators (minimal, run by default; extended, opt-in), `DocsGenerator` (Markdown, opt-in), `ModelGenerator` (Phase 7 first step — emits a real, executable `SELECT` for a dataset's computed fields and joined-in sources, qualifying every column reference by its source; distinct from `SQLGenerator`, which only ever emits schema DDL), `MermaidERDGenerator` (structifact.com build plan, Step 1, opt-in — see `docs/DECISION_HISTORY.md`'s "Two Different Numbering Systems Both Called 'Phase'" entry for why the site build plan is numbered in "Steps," never "Phases" — emits a Mermaid `erDiagram` per dataset: fields with type, `PK`/`FK` markers from `ConstraintSpec`, and one relationship line per `foreign_key` constraint, with crow's-foot cardinality read from the FK column's own `nullable`. Dataset-level `depends_on` is deliberately rendered only as a `%%` comment, never a relationship line — it carries no column mapping or cardinality, unlike a `foreign_key` constraint, and Mermaid's notation has no way to express a relationship without asserting one), and `JSONSchemaGenerator` (structifact.com build plan, Step 2, opt-in — the clearest single signal that Structifact isn't warehouse-specific: a straight mapping from `FieldSpec` to a JSON Schema document, `type` to JSON Schema type, `nullable: false` to the dataset-level `required` list, `accepted_values` to `enum`, `min_value`/`max_value` to `minimum`/`maximum`, `pattern` to `pattern`, plus `date`/`timestamp` to a `format` annotation using JSON Schema's own designated slot for that information. An "unknown"-typed field gets no `type` keyword at all — an absent `type` means "any type accepted," which is accurate here, rather than fabricating a default the way `SQLGenerator`'s `TEXT` fallback does. `ConstraintSpec` and `length`/`precision`/`scale` are deliberately out of scope for this first version — see the generator's own docstring for why. Its generated output is checked against the real JSON Schema 2020-12 meta-schema in tests, the same "verify against the real thing, not just the generator's own idea of correct syntax" discipline `MermaidERDGenerator`'s `mmdc` round-trip test uses).
