...
* `SQLGenerator` silently drops `FieldSpec.length` for string fields in generated DDL (always emits bare `TEXT`), unlike `decimal`, where `precision`/`scale` are honored — a real, verifiable gap, but not one any real-world validation round's reference materials have actually tested against yet (none included hand-built DDL)
