...
A third real example (`customer_credit`, motivating `AggregateRule` — see the dedicated section above) and a fourth, in-progress real example (`deliveries`) together confirmed a second, genuinely distinct text label some downstream dbt/catalog tooling expects per field, alongside `description` — not derived from it (the two real files that use it disagree on which one is the "fuller" label). `FieldSpec` gained `comment`, emitted by both dbt generators (`meta.comment`, alongside `role`/`source_field`) and — closing a gap that had stood since `catalog_extended` was first built — `ExtendedCatalogCSVGenerator`'s `comments` column, previously always blank since the IR had no source for it at all. Omitted when unset, same as every other optional text attribute.
