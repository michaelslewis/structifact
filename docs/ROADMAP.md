...
**A fourth real example (`deliveries`) found a genuine scale limit in AI extraction, and confirmed two structural patterns hold on real data neither had been tested against before.** This ticket was materially bigger again than any prior one — roughly 500 fields across four joined SAP tables, with a requirements document containing a later-added section overriding specific fields' `description`/adding a `comment` while leaving their earlier-declared role/type/length untouched. `discover --ai` never got the chance to be evaluated on the override-merge question at all: the response was cut off by `AnthropicLLMClient`'s output token cap, and a diagnostic attempt at a much higher cap was rejected outright by the Anthropic SDK (requests estimated to take over 10 minutes require streaming, which the client doesn't implement) — a real, mechanical scale boundary, logged in `FUTURE_WORK.md` as its own future paper-contract candidate rather than patched mid-round.

Rather than hand-model all ~500 fields, a representative 7-field sample was used to test two things directly against the real reference: a **chained join** (`vbup` joins to `lips` — a previously-joined source — not directly to the primary `likp`, a topology no prior example had exercised), and **`FieldSpec.comment`** (see "Recently Completed" above) against a second, independent real reference file. Both held with zero additional code changes to `ModelGenerator` — chained joins already work because generated `LEFT JOIN`s are emitted in declaration order, and a later `ON` clause can reference any already-joined table in standard SQL. Verified two ways: all 7 fields matched the real reference exactly (`description`, `role`, `comment`, `source_field`, catalog `comments`), and the generated model was executed against real DuckDB with data specifically designed to prove the chained join's `NULL`-preservation semantics (a row with no matching `vbup` row, and a row with no matching `lips` row at all, both came through correctly rather than being dropped).
