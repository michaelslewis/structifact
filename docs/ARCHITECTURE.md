...
FieldSpec
ConstraintSpec
SourceRef / JoinSpec / DedupRule / AggregateRule
...
Location: `structifact/ir.py` (Phase 7 — sources/joins milestone; `AggregateRule` added later via real-world use, see below)

These four concepts let one dataset be assembled from more than one underlying source, including the same physical table referenced multiple times under different roles.
...
SourceRef
    ...
    aggregate        optional AggregateRule (mutually exclusive with dedup)
...
AggregateRule
    group_by         columns the source is aggregated down to one row per
    aggregates       output column alias -> raw SQL aggregate expression
```

The `SourceRef.name` vs `.table` distinction is the reason this exists as several concepts rather than a simple table reference: `table` is the physical table; `name` is a logical alias for *this particular joined-in instance* of it. This is what lets the same physical table (e.g. a shared `partner_role` table) be joined into one dataset three separate times under three different roles (requested-by/billed-to/site-contact), each with its own `filter` and `dedup`/`aggregate` rule — `name` is what `JoinSpec.source` and `FieldSpec.source` both refer to, never `table` directly.

`DedupRule` and `AggregateRule` are the two mutually exclusive ways a joined source is collapsed to one row per key before the join happens — `SourceRef` may set at most one (checked in `validation.py`). `DedupRule` represents priority-based row selection, not a uniqueness constraint — given a group of rows sharing the same `partition_by` key(s), exactly one wins, chosen by `order_by` priority. Maps directly onto `ROW_NUMBER() OVER (PARTITION BY ... ORDER BY ...) = 1`, generated by `ModelGenerator` as its own CTE per source. `AggregateRule` instead aggregates every row in the group — `group_by` names the grouping columns, `aggregates` maps each output column alias to a raw SQL aggregate expression (a plain `SUM(col)`, or something more involved like a conditional sign-flip `CASE`). Maps directly onto a `GROUP BY <group_by> SELECT <group_by>, <aggregate expressions>` CTE. Found via real-world use (a real SAP-shaped account-summary source needing a joined source pre-aggregated before its join — see `DECISION_HISTORY.md`); deliberately requires zero changes to `ModelGenerator`'s own final-select shape, which stays a flat, ungrouped `SELECT` regardless of whether any joined source uses `dedup` or `aggregate`.

Like `expression` and `JoinSpec.on`, `SourceRef.filter`, `DedupRule.order_by`, and `AggregateRule.aggregates`' expressions are raw SQL fragments — trusted as-is, not parsed or validated. What validation.py *does* check about `sources`/`joins`: `SourceRef.name` values are unique within a dataset, every `JoinSpec.source` resolves to a declared source, every `DedupRule` has non-empty `partition_by`/`order_by`, every `AggregateRule` has a non-empty `group_by` and at least one non-blank aggregate, a `SourceRef` never sets both `dedup` and `aggregate`, and `JoinSpec.type` is one of the currently-supported types (`left`/`inner`).
...
Transformation-model generation (`model.py`, `ModelGenerator`, Phase 7 first step) — emits a real, executable `SELECT`, not DDL. For a dataset with `sources`/`joins`, builds the necessary CTEs (including `ROW_NUMBER()`-based dedup per `DedupRule`, or a `GROUP BY` pre-aggregation per `AggregateRule`) and qualifies every column reference by its source alias. Returns `None` — not an `Artifact` — for a dataset with no computed fields and no sources/joins, since there's nothing to transform. Not run by default.
Mermaid ERD generation (`mermaid_erd.py`, `MermaidERDGenerator`, structifact.com build plan Step 1 — see `docs/DECISION_HISTORY.md`'s "Two Different Numbering Systems Both Called 'Phase'" entry for why the site build plan uses "Step N," not "Phase N") — emits a Mermaid `erDiagram` per dataset: fields with type, `PK`/`FK` markers from `ConstraintSpec`, one relationship line per `foreign_key` constraint with crow's-foot cardinality read from the FK column's own `nullable`. Dataset-level `depends_on` is rendered only as a `%%` comment, never a relationship line, since it carries no column mapping or cardinality. Not run by default.
JSON Schema generation (`json_schema.py`, `JSONSchemaGenerator`, structifact.com build plan Step 2) — a straight mapping from `FieldSpec` to a JSON Schema document: `type` to JSON Schema type, `nullable: false` to `required`, `accepted_values` to `enum`, `min_value`/`max_value` to `minimum`/`maximum`, `pattern` to `pattern`, plus `date`/`timestamp` to a `format` annotation. An "unknown"-typed field gets no `type` keyword at all rather than a fabricated default. `length`/`precision`/`scale` and every `ConstraintSpec` type are deliberately out of scope for this first version. Not run by default.
...
`OPTIONAL_GENERATORS` — available, but not run unless explicitly requested via `structifact generate -g <name>`. Reserved for generators that depend on assumptions Structifact cannot make for every user (the extended catalog generator, docs, and the transformation model), or that are new enough that they shouldn't silently change existing default output for anyone (the transformation model, `MermaidERDGenerator`, and `JSONSchemaGenerator` — the last two from the structifact.com build plan, Steps 1 and 2 — see `docs/DECISION_HISTORY.md`'s "Two Different Numbering Systems Both Called 'Phase'" entry for why that plan uses "Step N," never "Phase N").
